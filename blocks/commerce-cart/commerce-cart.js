...
function renderProductPlaceholder(ctx, item, anchorWrapper) {
  const placeholder = document.createElement('span');
  placeholder.className = 'commerce-cart__product-placeholder';
  placeholder.setAttribute('aria-label', item.name || item.sku || 'Product');
  placeholder.textContent = (item.name || item.sku || 'P')
    .split(/\s+/)
    .map((word) => word[0])
    .join('')
    .slice(0, 2)
    .toUpperCase();
  anchorWrapper.append(placeholder);
        ...
        Thumbnail: (ctx) => {
          ...
          if (defaultImageProps?.src) {
            try {
              ...
              console.warn('Unable to render cart product image.', error);
              renderProductPlaceholder(ctx, item, anchorWrapper);
            }
          } else {
            renderProductPlaceholder(ctx, item, anchorWrapper);
          }
