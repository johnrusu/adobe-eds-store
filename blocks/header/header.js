...
function ensureNavStructure(nav) {
  ...
  const brandWrapper = nav.children[0].querySelector('.default-content-wrapper')
    || nav.children[0].appendChild(document.createElement('div'));
  if (!brandWrapper.textContent.trim()) {
    brandWrapper.innerHTML = '<p><a href="/">Adobe Store</a></p>';
  }
}

function clearCartForStoreChange() {
  document.cookie = 'DROPIN__CART__CART-ID=; path=/; Max-Age=0';
  ...
  localStorage.removeItem('DROPIN__CART__STORE_VIEW');
}
...
export default async function decorate(block) {
    ...
    if (state) {
      await withLoadingState(searchPanel, searchButton, async () => {
          ...
          slots: {
            ProductImage: (ctx) => {
              ...
              if (!defaultImageProps?.src?.trim()) {
                anchorWrapper.className = 'nav-search-product-placeholder';
                anchorWrapper.textContent = (product.name || product.sku)
                  .split(/\s+/)
                  .slice(0, 2)
            ...
            Footer: async (ctx) => {
              ...
              ctx.onChange((next) => {
                viewAllResultsButton?.setProps((prev) => ({
                  ...prev,
                  href: `${rootLink('/products/search')}?q=${encodeURIComponent(next.variables?.phrase || '')}`,
                }));
              });
            },
          },
        ...
        searchForm.addEventListener('submit', (e) => {
          ...
          if (query.length) {
            window.location.href = `${rootLink('/products/search')}?q=${encodeURIComponent(query)}`;
          }
        });
  ...
  /** Store View Switcher */
  const storeViews = getConfigValue('store-views') || [];
  if (Array.isArray(storeViews) && storeViews.length > 1) {
    const activeStoreCode = localStorage.getItem('store-view')
      || getConfigValue('headers.all.Store')
      || 'default';
    const activeStore = storeViews.find(({ code }) => code === activeStoreCode) || storeViews[0];
    const storeSwitcher = document.createRange().createContextualFragment(`
      <div class="store-switcher nav-tools-wrapper">
        <button type="button" class="nav-store-button" aria-label="Change store or language" aria-expanded="false">
          <span class="store-icon" aria-hidden="true"></span>
          <span class="store-label">${activeStore.currency}</span>
        </button>
        <ul class="store-switcher-panel nav-tools-panel" role="listbox" aria-label="Store views">
          ${storeViews.map((store) => `
            <li role="option" aria-selected="${store.code === activeStoreCode}" data-store-code="${store.code}" class="store-option${store.code === activeStoreCode ? ' active' : ''}">
              <span class="store-option-label">${store.label}</span>
              <span class="store-option-currency">${store.currency}</span>
            </li>`).join('')}
        </ul>
      </div>
    `);
    navTools.append(storeSwitcher);

    const storeButton = navTools.querySelector('.nav-store-button');
    const storePanel = navTools.querySelector('.store-switcher-panel');
    const toggleStorePanel = (show) => {
      storeButton.setAttribute('aria-expanded', show ? 'true' : 'false');
      storePanel.classList.toggle('nav-tools-panel--show', show);
    };

    storeButton.addEventListener('click', () => {
      toggleStorePanel(storeButton.getAttribute('aria-expanded') !== 'true');
    });
    storePanel.querySelectorAll('.store-option').forEach((option) => {
      option.addEventListener('click', () => {
        const { storeCode } = option.dataset;
        if (storeCode === activeStoreCode) {
          toggleStorePanel(false);
          return;
        }

        localStorage.setItem('store-view', storeCode);
        sessionStorage.removeItem('config');
        clearCartForStoreChange();
        window.location.reload();
      });
    });
    document.addEventListener('click', (event) => {
      if (!storeButton.contains(event.target) && !storePanel.contains(event.target)) {
        toggleStorePanel(false);
      }
    });
  }
