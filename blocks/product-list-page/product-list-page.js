...
function renderProductImagePlaceholder(ctx, product, wrapper) {
  wrapper.classList.add('product-list-page__image-placeholder');
  wrapper.setAttribute('aria-label', `View ${product.name || product.sku}`);

  const monogram = document.createElement('span');
  monogram.className = 'product-list-page__image-placeholder-mark';
  monogram.setAttribute('aria-hidden', 'true');
  monogram.textContent = (product.name || product.sku)
    .split(/\s+/)
    .slice(0, 2)
  ...
  label.className = 'product-list-page__image-placeholder-label';
  label.textContent = product.sku || 'Adobe Commerce';

  wrapper.append(monogram, label);
...
export default async function decorate(block) {
  ...
  const getAddToCartButton = (product) => {
    ...
    const addToCartText = labels.Global?.AddProductToCart || 'Add to cart';
    const addToCartLabel = `${addToCartText}: ${productName}`;
