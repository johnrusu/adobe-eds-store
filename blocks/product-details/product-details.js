...
export default async function decorate(block) {
  ...
  if (!product?.images?.some(({ url }) => url)) {
    $gallery.replaceChildren(createProductVisual(product));
    $galleryMobile.replaceChildren(createProductVisual(product));
  }
  ...
  events.on(
    'pdp/valid',
    (valid) => {
      // Disable add-to-cart when the selection is invalid or the product is out of stock.
      addToCart.setProps((prev) => ({
        ...prev,
