 ...
 */
function applySelectedStoreHeaders(configJSON) {
  const selectedStore = window.localStorage.getItem('store-view') || 'default';

  if (configJSON?.public?.default?.headers?.all) {
    configJSON.public.default.headers.all.Store = selectedStore;
  }

  if (configJSON?.public?.default?.headers?.cs) {
    configJSON.public.default.headers.cs['Magento-Store-Code'] = selectedStore;
    configJSON.public.default.headers.cs['Magento-Store-View-Code'] = selectedStore;
  }
...
export async function getConfigFromSession() {
  ...
  try {
    ...
    if (!parsedConfig?.public?.default?.headers?.cs) {
      throw new Error('Config missing Catalog Service headers');
    }

    const normalizedConfig = applySelectedStoreHeaders(parsedConfig);
    window.sessionStorage.setItem('config', JSON.stringify(normalizedConfig));
    ...
    applySelectedStoreHeaders(configJSON);
